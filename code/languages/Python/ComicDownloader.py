...
class HentaiVN:
    ...
    def get_data(self, link):
        ...
        # </td>
        list_chapters = crawl_data.find("tbody").find_all(class_="chuong_td")
        ...
        # [{'name:': .., 'link': ..}, {}, ..]
        data_list_chapters = list(
            map(
                lambda x: dict(
                    name=x.text.strip(), link=self.hostname + x.find("a")["href"]
                ),
                list_chapters,
            )
        )
    ...
    def download_image(self, data_chapter):
        ...
        for (
            ...
            downloaded_file_names.append(
                object_file_handle.download_file(link_img, file_name)
            )
        ...
        other_files = [
            "favicon{}.ico".format(file_zip_name),
            "background{}.jpg".format(file_zip_name),
        ]
        # make favicon file and background image file
        object_file_handle.copy_file(
            PATH_PARENT + "/" + "html_maker/favicon.ico",
            object_file_handle.get_current_directory() + "/" + other_files[0],
        )
        object_file_handle.copy_file(
            PATH_PARENT + "/" + "html_maker/background.jpg",
            object_file_handle.get_current_directory() + "/" + other_files[1],
        )
        other_files.append(
            object_file_handle.make_html_file(
